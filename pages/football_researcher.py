...
def _save_state_to_supabase(session_id: str = None) -> bool:
    ...
    try:
        ...
        state_to_save = {
            ...
            "exploration_results": st.session_state.get("exploration_results", {}),
            "sweep_results": st.session_state.get("sweep_results", {}),
            "exploration_analysis": st.session_state.get("exploration_analysis", {}),
            "avenues_to_explore": st.session_state.get("avenues_to_explore", []),
...
Respond with JSON:
{
    ...
    # Get best drift from drift_analysis or exploration data
    drift_analysis = parsed.get("drift_analysis", {})
    default_drift = drift_analysis.get("best_drift")
    
    # If no drift in analysis, calculate from exploration
    if not default_drift:
        ...
        for d in drift_dist:
            ...
            if drift_pl > best_drift_pl:
                ...
                default_drift = d.get("DRIFT IN / OUT")
        if not default_drift:
            default_drift = "IN"  # Ultimate fallback
    
    # Ensure every avenue has promising_drift
...
def _generate_fallback_avenues(exploration: Dict, pl_column: str) -> Dict:
    ...
    if len(avenues) < 4:
        ...
        for m in sorted(mode_dist, key=lambda x: x.get(f"mean_{pl_column}", 0) or 0, reverse=True)[:2]:
            mode = m.get("MODE")
    ...
    if len(avenues) < 4:
        ...
        for m in sorted(market_dist, key=lambda x: x.get(f"mean_{pl_column}", x.get("mean_BO 2.5 PL", 0)) or 0, reverse=True)[:3]:
            market = m.get("MARKET")
            mean_pl = m.get(f"mean_{pl_column}", m.get("mean_BO 2.5 PL", 0))
            count = m.get("_count", 0)
...
def run_agent(is_new_run: bool = True):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        should_save = (
            ...
            (datetime.utcnow() - last_save).total_seconds() > STATE_AUTO_SAVE_INTERVAL
        )
