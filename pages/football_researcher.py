...
def _resolve_pl_column(user_text: str, ctx: Dict[str, Any]) -> str:
    """Resolve which PL/outcome column the user meant.

    Key rule: NEVER match the generic 'PL' just because the user wrote 'PL strategy'.
    We only return 'PL' when the user explicitly asks for overall/total PL, or they
    did not specify any specific market/outcome term.
    """
    t_raw = user_text or ""
    t = _normalize(t_raw)
    outcomes: List[str] = ((ctx.get("derived") or {}).get("outcome_columns") or [])
    outcomes = [o for o in outcomes if isinstance(o, str) and o.strip()]
    outcomes_lower = {o.strip().lower(): o for o in outcomes}

    # Explicit aliases / synonyms (add freely, but keep leakage-safe)
    aliases = {
        "btts": "BTTS PL",
        ...
        "bo1.5 fhg": "BO1.5 FHG PL",
        "fh under 0.5": "LFGHU0.5 PL",
        "fhu0.5": "LFGHU0.5 PL",
        "lfghu0.5": "LFGHU0.5 PL",
        "lfhgu0.5": "LFGHU0.5 PL",  # common typo (missing 'g')
    }

    # If an alias matches, return it (only if it's in the allowed outcomes list)
    for k, v in aliases.items():
        if k in t:
            if v.strip().lower() in outcomes_lower:
                return outcomes_lower[v.strip().lower()]
            return v

    # Compact form match against all non-generic outcomes
    t_compact = re.sub(r"[^a-z0-9]+", "", t.lower())

    non_generic = [o for o in outcomes if o.strip().lower() != "pl"]
    # Prefer longer/more specific strings first
    non_generic_sorted = sorted(non_generic, key=lambda c: -len(c))

    for col in non_generic_sorted:
        col_compact = re.sub(r"[^a-z0-9]+", "", col.lower())
        if col_compact and (col_compact in t_compact):
            return col

    # Fuzzy match for typos (e.g. 'LFHGU0.5' -> 'LFGHU0.5 PL')
    best = None
    best_score = 0.0
    for col in non_generic_sorted:
        col_compact = re.sub(r"[^a-z0-9]+", "", col.lower())
        if not col_compact:
            continue
        score = difflib.SequenceMatcher(None, t_compact, col_compact).ratio()
        if score > best_score:
            best_score = score
            best = col
    ...
    # Only return generic PL if explicitly asked
    wants_generic = any(x in t for x in ["overall pl", "total pl", "generic pl", "plain pl", "all pl"])
    # If they didn't specify any market-like token, assume generic PL.
    mentioned_specific = any(tok in t for tok in ["bo", "btts", "shg", "lu", "fh", "u0.5", "o2.5", "o 2.5", "2.5", "1.5"])
    if wants_generic or (not mentioned_specific and "pl" in t):
        # If PL exists in outcomes, use exact column name from sheet
        if "pl" in outcomes_lower:
            return outcomes_lower["pl"]
        return "PL"

    # Fallback: if BO 2.5 exists use it; else first non-generic; else PL.
    if "bo 2.5 pl" in outcomes_lower:
        return outcomes_lower["bo 2.5 pl"]
    if non_generic_sorted:
        return non_generic_sorted[0]
    return outcomes_lower.get("pl", "PL")
...
def _maybe_start_narrated_pl_research(user_text: str) -> bool:
    ...
    plan = ["pl_lab"]
    if auto_diag:
        plan += ["subgroup_scan", "bracket_sweep", "hyperopt_pl_lab"]
    st.session_state["agent_session_plan"] = plan
    st.session_state["agent_session_plan_index"] = 0
...
def _autopilot_tick():
    ...
    # Decide next step (deterministic chaining)
    # ------------------------------------------------------------
    # Users explicitly asked: "if no rules pass, automatically run diagnostics".
    # The most reliable way to guarantee that is a fixed plan, not an LLM decision.
    plan = st.session_state.get("agent_session_plan") or []
    if isinstance(plan, list) and plan:
        # Update any learned filters from diagnostic outputs
        try:
            cur_task_type = job.get("task_type")
            if cur_task_type == "subgroup_scan":
                top_groups = (payload or {}).get("top_groups") or []
                if top_groups:
                    g0 = top_groups[0]
                    group_vals = g0.get("group") or {}
                    # Merge categorical group values into row_filters
                    rf = dict(st.session_state.get("agent_session_row_filters") or {})
                    for k, v in (group_vals or {}).items():
                        if v is None:
                            continue
                        rf[str(k)] = v
                    st.session_state["agent_session_row_filters"] = rf
            elif cur_task_type == "bracket_sweep":
                top_brackets = (payload or {}).get("top_brackets") or []
                if top_brackets:
                    b0 = top_brackets[0]
                    col = b0.get("col")
                    lo = b0.get("lo")
                    ...
                    if col is not None and lo is not None and hi is not None:
                        rf = dict(st.session_state.get("agent_session_row_filters") or {})
                        rf[str(col)] = {"min": float(lo), "max": float(hi)}
                        st.session_state["agent_session_row_filters"] = rf
            elif cur_task_type == "hyperopt_pl_lab":
                # If hyperopt returns a best_rule, store it for user display.
                best = (payload or {}).get("best_rule")
                if best:
                    st.session_state["agent_session_best_rule"] = best
        except Exception:
            # Never let chaining fail the UI
            pass

        # Align plan index with the actual task we just completed
        plan_idx = int(st.session_state.get("agent_session_plan_index", 0) or 0)
        if plan_idx < len(plan) and plan[plan_idx] != job.get("task_type"):
            if job.get("task_type") in plan:
                plan_idx = plan.index(job.get("task_type"))
                st.session_state["agent_session_plan_index"] = plan_idx

        # Stop immediately if we found passing rules (job achieved the goal)
        if passing_rules:
            st.session_state["agent_session_active"] = False
            st.session_state.active_job_id = ""
            _append("assistant", "✅ Rules passed the gates — autopilot stopping here.")
            return

        # If there are more steps in the plan, submit the next job.
        if plan_idx + 1 < len(plan):
            next_task = plan[plan_idx + 1]
            ...
            if rf:
                base_params["row_filters"] = rf

            # If we *don't* yet have row_filters, constrain by MARKET when possible.
            if (not rf) and base_params.get("pl_column") and base_params["pl_column"] != "PL":
                mkt = str(base_params["pl_column"]).replace(" PL", "").strip()
                if mkt:
                    base_params["row_filters"] = {"MARKET": mkt}

            # Task-specific defaults
            if next_task == "subgroup_scan":
                params = dict(base_params)
                params.update({
                    "group_cols": ["MODE", "MARKET", "LEAGUE", "BRACKET", "DRIFT IN / OUT"],
                    "max_groups": 120,
                    "min_rows": 60,
                    "top_n": 40,
                })
            elif next_task == "bracket_sweep":
                params = dict(base_params)
                params.update({
                    "sweep_cols": ["DIFF", "% DRIFT", "IMPLIED ODDS", "ACTUAL ODDS", "STRIKE RATE", "NO GAMES", "WINS"],
                    "n_bins": 10,
                    "top_n": 50,
                    "min_rows": 60,
                })
                ...
                params = dict(base_params)
                # hyperopt can be expensive; keep trials sensible and let Modal cap runtime.
                params.update({
                    "hyperopt_trials": 120,
                })
            else:
            ...
            sub = _run_tool("submit_job", {"task_type": next_task, "params": params})
            next_job_id = (sub or {}).get("job_id")
            if next_job_id:
                st.session_state["agent_session_plan_index"] = plan_idx + 1
                st.session_state.active_job_id = next_job_id
                _append("assistant", f"🧪 Auto diagnostics: started **{next_task}**. Job ID: {next_job_id}")
                return

            _append("assistant", f"⚠️ Auto diagnostics tried to start **{next_task}** but submit_job failed: {sub}")
            st.session_state["agent_session_active"] = False
    st.session_state.active_job_id = ""
