...
# Read ALL keys from Secrets (safer). If you truly want to hardcode later, replace these lines.
...
# --- Make.com webhook: your provided URL (hard fallback if not set in Secrets) ---
MAKE_WEBHOOK_URL     = _secret("MAKE_WEBHOOK_URL", "https://hook.eu2.make.com/sd6mavmdqbyxx0mi5cvbv7ndimyfzh2u")

# --- GitHub repo/branch for PRs ---
# Prefer to set these in Secrets:
#   GITHUB_REPO = "username/reponame"  (e.g., "samueljcoombs-rgb/JarvisSC")
#   GITHUB_BRANCH = "main"
GITHUB_REPO          = _secret("GITHUB_REPO", "username/reponame")  # CHANGE this to your real "user/repo"
...
TOOL_SPECS = [
    ...
    {"name":"workout_today", "description":"Read today's workout plan from sheet",
     "parameters":{"type":"object","properties":{}}},
    {"name":"read_code", "description":"Read app.py (or another file) to understand system behavior.",
...
def agent_chat(history: List[Dict[str, Any]]):
    ...
    system_msg = {
        ...
        "content": (
            ...
            "Use tools when helpful. Be concise. Persistent memories:\n\n"
            + memory.recent_summary()
        )
    }

    # First request (lighter model to reduce rate limits)
    ...
    if resp is None:
        return "Sorry — I’m temporarily rate-limited. Try again in ~10s."

    msg = resp.choices[0].message
    all_messages = [system_msg] + history + [msg]

    # Tool-call loop (cap loops)
    MAX_TOOL_LOOPS = 2
    loops = 0
...
# ---------- UI ----------
st.title("🤖 JARVIS")
st.caption("Agent + life dashboard (memory, code reading, auto-PR)")
...
with col2:
    ...
    st.write(dt.date.today().isoformat())
    st.markdown('</div>', unsafe_allow_html=True)
...
if todos:
    for r in todos:
        ...
        if new_val != checked:
            st.toast(todo_toggle(r.get("Task",""), new_val))
else:
    st.write("(No items)")
st.markdown('</div>', unsafe_allow_html=True)
...
with colB:
    new_fact = st.text_input("Remember this (press Enter):", key="remember_box",
                             label_visibility="collapsed", placeholder="e.g., I prefer evening workouts")
    if new_fact:
...
# Developer Mode — send request to Make.com to open a PR
st.markdown("### Developer Mode")
dev_req = st.text_area("Describe the code change you want (e.g., 'move To-Do above Feeds and increase font size')", height=90)
if st.button("🚀 Propose PR"):
    if not MAKE_WEBHOOK_URL:
        st.error("MAKE_WEBHOOK_URL is not set.")
    else:
        try:
            ...
            if r.status_code == 200:
                st.success("Sent to Make.com — a Pull Request will appear in your repo.")
            else:
                st.error(f"Webhook error {r.status_code}: {r.text[:180]}")
        except Exception as e:
