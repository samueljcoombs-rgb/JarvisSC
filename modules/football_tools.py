...
# ============================================================
# NEW: One-shot "source of truth" context fetch + parsing
# ============================================================
...
def _extract_outcome_columns(col_defs: List[Dict[str, str]]) -> List[str]:
    ...
    for r in col_defs or []:
        ...
        if col and role == "outcome":
            out.append(col)
    # also include any "* PL" columns even if missing in definitions
    return out


def get_research_context(limit_notes: int = 20) -> Dict[str, Any]:
    """
    Fetches all relevant tabs + returns derived constraints (ignored/outcome columns).
    """
    ...
    return {
        ...
        "recent_notes": notes,
        "derived": {
            "ignored_columns": ignored_cols,          # e.g. ["Result", "HOME FORM"]
            "outcome_columns": outcome_cols,          # from column_definitions role==outcome
        },
        "ts": _now_iso(),
    }
...
# ============================================================
# NEW: One-click BTTS lab starter that passes Sheet-derived rules
# ============================================================
...
def start_btts_lab(
    ...
    return submit_job("btts_lab", params)
